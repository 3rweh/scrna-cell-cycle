...
```{r load_data, echo=TRUE}
cc.data <- readRDS("/Users/erikh/Desktop/git/scrna-cell-cycle/embryonic_data/EMTAB2805.rds")
...
prop.zeros
# filter out genes that are sparsely and lowly expressed in both data sets
filter = apply(counts, 1, function(x) length(x[x>5])>=3)
filter_cc = apply(exprs, 1, function(x) length(x[x>5])>=3)
counts = counts[filter,]
exprs = exprs[filter_cc,]
dim(counts)
dim(exprs)
...
prop.zeros <- sum(counts==0)/length(counts)
prop.zeros
prop.zeros_cc <- sum(exprs==0)/length(exprs)
prop.zeros_cc
...
## Normalization using SCnorm for Glioblastoma
...
## Normalization using SCnorm for Buettner data
```{r normalization, fig.width=8, fig.height=6}
## count-depth relationship for all genes
Conditions = rep(c(1), each=288)
countDeptEst_cc <- plotCountDepth(Data = exprs, Conditions = Conditions,
                               FilterCellProportion = .1, NCores=4)

DataNorm_cc <- SCnorm(Data = exprs, Conditions = Conditions,
                   PrintProgressPlots = FALSE,
                   FilterCellNum = 10,
                   NCores=4)
exprs <- SingleCellExperiment::normcounts(DataNorm_cc)
summary(colSums(exprs))
dim(exprs)
```
