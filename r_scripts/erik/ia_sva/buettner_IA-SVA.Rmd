...
plot(iasva.sv_buet[,c(1,7)], main="IA-SVA_7", pch=21, xlab="SV1", ylab="SV7",
     col=color.vec[Cluster_buet_7], bg=color.vec[Cluster_buet_7])

# Cluster based on SVA6
...
plot(iasva.sv_buet[,c(1,6)], main="IA-SVA_6", pch=21, xlab="SV1", ylab="SV6",
     col=color.vec[Cluster_buet_6], bg=color.vec[Cluster_buet_6])

# Cluster based on SVA5
...
plot(iasva.sv_buet[,c(1,5)], main="IA-SVA_5", pch=21, xlab="SV1", ylab="SV5",
     col=color.vec[Cluster_buet_5], bg=color.vec[Cluster_buet_5])

# Cluster based on SVA4
...
plot(iasva.sv_buet[,c(1,4)], main="IA-SVA_4", pch=21, xlab="SV1", ylab="SV4",
     col=color.vec[Cluster_buet_4], bg=color.vec[Cluster_buet_4])

# Cluster based on SVA3
...
plot(iasva.sv_buet[,c(1,3)], main="IA-SVA_3", pch=21, xlab="SV1", ylab="SV3",
     col=color.vec[Cluster_buet_3], bg=color.vec[Cluster_buet_3])

# Cluster based on SVA2
...
plot(iasva.sv_buet[,c(1,2)], main="IA-SVA_2", pch=21, xlab="SV1", ylab="SV2",
     col=color.vec[Cluster_buet_2], bg=color.vec[Cluster_buet_2])

# Cluster based on SVA1
...
plot(iasva.sv_buet[,c(1,2)], main="IA-SVA_1", pch=21, xlab="SV1", ylab="SV2",
     col=color.vec[Cluster_buet_1], bg=color.vec[Cluster_buet_1])

# Correlation between the number of detected genes/total library and the surrogate variables
...
pdf(paste0("/Users/erikh/Desktop/git/scrna-cell-cycle/r_scripts/erik/ia_sva/sc_output/Clustering_analyses_figure3_sv1_buet.pdf"))
r2.results_buet <- study_R2(summ_exp_buet, iasva.sv_buet, selected.svs=2, no.clusters=2)
dev.off()

marker.counts.SV1_buet <- find_markers(summ_exp_buet, 
                            as.matrix(iasva.sv_buet[,c(1)]), rsq.cutoff = 0.4)
marker.counts.SV1.long_buet <- find_markers(summ_exp_buet, 
                              as.matrix(iasva.sv_buet[,c(1)]), rsq.cutoff = 0.45)
nrow(marker.counts.SV1_buet) 
nrow(marker.counts.SV1.long_buet)

anno.col2_buet <- data.frame(Cluster=Cluster_buet, SV1=iasva.sv_buet[,1])
rownames(anno.col2_buet) <- colnames(marker.counts.SV1_buet)
head(anno.col2_buet)

cluster.col_buet <- color.vec[1:2]
names(cluster.col_buet) <- as.vector(levels(Cluster_buet_1))
anno.colors_buet <- list(Cluster=cluster.col_buet)
anno.colors_buet

pheatmap(log(marker.counts.SV1_buet+1), show_colnames =FALSE, 
         clustering_method = "ward.D2", cutree_cols = 2, annotation_col = anno.col2_buet,
         annotation_colors = anno.colors_buet)

pheatmap(log(marker.counts.SV1.long_buet+1), show_colnames =FALSE, 
         clustering_method = "ward.D2", cutree_cols = 2, annotation_col = anno.col2_buet,
         annotation_colors = anno.colors_buet)

gene.list_buet_SV1 <- rownames(marker.counts.SV1_buet)
write.table(gene.list_buet_SV1, file = paste0("/Users/erikh/Desktop/git/scrna-cell-cycle/r_scripts/erik/ia_sva/sc_output/CC_genes.short_buet_SV1.txt"),
            col.names =F, row.names = F, quote = F)

gene.list.long_buet_SV1 <- rownames(marker.counts.SV7.long_buet)
write.table(gene.list.long_buet_SV1, file = paste0("/Users/erikh/Desktop/git/scrna-cell-cycle/r_scripts/erik/ia_sva/sc_output/CC_genes.long_buet_SV1.txt"),
            col.names =F, row.names = F, quote = F)

```

## Find marker genes for the detected heterogeneity (SV7).
  
```{r find_markers_SV7, echo=TRUE, fig.width=6, fig.height=16}
...
cluster.col_buet <- color.vec[1:2]
names(cluster.col_buet) <- as.vector(levels(Cluster_buet_7))
anno.colors_buet <- list(Cluster=cluster.col_buet)
anno.colors_buet
...
gene.list_buet_SV7 <- rownames(marker.counts.SV7_buet)
write.table(gene.list_buet_SV7, file = paste0("/Users/erikh/Desktop/git/scrna-cell-cycle/r_scripts/erik/ia_sva/sc_output/CC_genes.short_buet_SV7.txt"),
            col.names =F, row.names = F, quote = F)

gene.list.long_buet_SV7 <- rownames(marker.counts.SV7.long_buet)
write.table(gene.list.long_buet_SV7, file = paste0("/Users/erikh/Desktop/git/scrna-cell-cycle/r_scripts/erik/ia_sva/sc_output/CC_genes.long_buet_SV7.txt"),
            col.names =F, row.names = F, quote = F)
