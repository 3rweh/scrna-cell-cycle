---
title: "Detecting cell-cycle stage difference in Buettner mESC cells cells"
...
Frequently, the first principal component of log-transformed scRNA-Seq read counts is highly correlated with the number of detected genes (e.g., r > 0.9). Here, we calculate the number of detected genes for Mus musculus mESC cells, which will be used as an known factor in the IA-SVA analyses. 
...
## Run IA-SVA
Here, we run IA-SVA using Geo_Lib_Size as a known factor and identify seven hidden factors. 
...
Here cyclone is used to assign cell cycle phases. Vioplot visualizes how well the different SVs separate the cell cycle phases. 
...
associated with SV2, since this had the lowest KS-value (multiple testing adjusted p-value < 0.05,
...
## Adding of Seurat gene list for comparison
```{r convert_names}
...
Match the gene list used in mmu.pairs for Cyclone (G1=12052 paired genes, S=6459 paired genes, G2M=9981 paired genes) and Seurat (S=40 and G2M=52) with the long gene lists from different SVs.
```{r}
#unlists the mmu.pairs and creat unique lists of genes for G1, S and G2M
...
#Matching the lists using intersect()
...
length(ccg2m_match_sv2)
list(ccg2m_match_sv2)
length(seug2m_match_sv2)
list(seug2m_match_sv2)
length(ccg2m_match_sv3)
list(ccg2m_match_sv3)
length(seug2m_match_sv3)
list(seug2m_match_sv3)
length(ccg2m_match_sv5)
list(ccg2m_match_sv5)
length(seug2m_match_sv5)
list(seug2m_match_sv5)

```
Here we get the matched genes in the predicted G2M phase for three different SVs mapping against cyclone (631 genes in G2M) and Seurat (52 genes in G2M). 40 Cyclone hits and 20 Seurat hits for sv2, 7 Cyclone hits and 1 Seurat hit for sv3, 40 Cyclone hits and 10 Seurat hits for sv5. Interesting note: 17 genes are shared between Cyclone and Seurat for sv2, 1 gene is shared between Cyclone and Seurat for sv3, 8 genes are shared between Cyclone and Seurat for sv5.