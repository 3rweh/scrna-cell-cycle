...
table(cc$phase)
test <- table(M$cell_cycle_stage)

library(stringr)
str_replace_all(M$cell_cycle_stage, "[\r\n]" , "")

test1 <- table(test)


# removes header
cat(test1, file = "actual.txt")

write(test1, file = "actual.txt",
      ncolumns = if(is.character(test1)) 1 else 5,
      append = FALSE, sep = " ")

# adds newline in the beginning
sink('actual.txt')
print(test,right=F)
sink()
