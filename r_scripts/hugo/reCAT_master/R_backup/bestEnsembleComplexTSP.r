...
# name = ""
# threads = 1
source("reCAT_master/R/highCorFind.r")
...
bestEnsembleComplexTSP <- function(test_exp, stageIdxSmp = NULL, TSPFold = 2, beginNum = 10, endNum = 198, output = TRUE, debug = FALSE, name = "", threads = 1, clustMethod = "GMM", base_cycle_range = c(6:9), step_size = 2, max_loop = NULL, max_num = 300)
{
  ...
  #print(baseResult)
	...
	else
	{
	  print("make cluster")
		cl <- makeCluster(threads)
		print("registers cluster")
		registerDoParallel(cl)
		print("finished")
		
		#resultLst <- foreach(clust_num_EM = (beginNum + 1): endNum, .combine='rbind') %dopar% 
		#seq(from = beginNum+step_size, to = endNum, by = step_size), .combine = 'rbind') %dopar%
		resultLst <- foreach(clust_num_EM = seq_list, .combine = 'rbind') %dopar% 
		{
		  print('Loading source')
		  source("reCAT_master/R/highCorFind.r")
		  print('Calling highCorFind function')
			highCorTour <- highCorFind(clust_num_EM, test_exp, stageIdxSmp, TSPFold = TSPFold, clustMethod = clustMethod, debug = debug)
	...
	if (debug == TRUE)
		message("print in ", paste("reCAT_master/R/bestEnsembleComplexTSP", beginNum, "-", endNum, name, ".pdf"))
		
	if (output == TRUE)
	{
		save(finalHighCorLst, highCorLst, resultLst, normalizedResultLst, ensembleResultLst, file = paste("reCAT_master/R/bestEnsembleComplexTSP", beginNum, "-", endNum, name, ".RData"))
		
		pdf(paste("reCAT_master/R/bestEnsembleComplexTSP", beginNum, "-", endNum, name, ".pdf"))
