ensIDToName_human <- as.matrix(read.table("reCAT_master/ensIDToName(Human).txt",header=FALSE,sep="\t",comment="",quote=""))
ensIDToName_mouse <- as.matrix(read.table("reCAT_master/ensIDToName(Mouse).txt",header=FALSE,sep="\t",comment="",quote=""))
hcyclebaseGeneList <- as.matrix(read.table("reCAT_master/cyclebaseGeneList",header=T,sep="\t",comment="",quote=""))[,1]
