...
cc_counts <- get_test_exp(t(cc_counts))
dim(cc_counts)
# Transposes the matrix for future use
cc_counts <- t(cc_counts)
```
...
score_result <- get_score(cc_counts)
...
# Checks the direction of the cycle (the order)
if(cycle_start >= length(ordIndex))
{
	myord = c(rdata[cycle_start,1]:1, (length(ordIndex):(rdata[cycle_start,1]+1)))
} else {
	myord = c(rdata[cycle_start,1]:1, ((rdata[cycle_start,1]+1):length(ordIndex)))
}
# myord = Time series 
# clsnum = Class-number (3 = G1, S, G2)
hmm_result <- get_hmm_order(bayes_score = score_result$bayes_score, mean_score = score_result$mean_score, ordIndex = ordIndex, cls_num = clsnum, myord = myord, rdata = rdata)

g1_c = sum(hmm_result == 1)
s_c = sum(hmm_result == 2)
g2m_c = sum(hmm_result == 3)

phase_counts = t(data.frame(g1_c , s_c, g2m_c))
